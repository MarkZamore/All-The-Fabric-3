#!/usr/bin/env python3
"""Generate the ATF3 pack distribution manifest and staged release assets.
...
Behavior:
    * Collects the shipped file set: everything under the roots
      ...
      shaderpacks/, configureddefaults/, journeymap/, simple-rpc/, launcher/
      (recursively, absent roots are skipped) plus the root files
      portable-pack.json and the client jar
...
# "journeymap" and "simple-rpc" are this pack's additions to the list its
# sibling packs use. Both are folders their mods read directly - JourneyMap
# keeps its per-world map settings and icon sets there, Simple RPC its Discord
# presence templates - and both arrive in the pack's own overrides. A root
# missing from this list is not merely absent from the manifest: the launcher
# holds a pack folder to exactly what the manifest names and deletes the rest,
# so they would be swept away and nothing anywhere would say why.
SCAN_ROOTS = ("mods", "config", "kubejs", "scripts", "defaultconfigs", "data",
              "resourcepacks", "shaderpacks", "configureddefaults", "journeymap",
              "simple-rpc", "launcher")
